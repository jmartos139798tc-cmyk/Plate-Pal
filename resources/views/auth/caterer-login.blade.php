    ...
    <div class="w-full lg:w-1/2 flex flex-col px-4 sm:px-8 lg:px-20 py-6 sm:py-10 bg-white">
        <div class="flex items-center justify-between mb-8 sm:mb-16">
            ...
            <a href="{{ route('home') }}" class="inline-flex items-center gap-1 text-xs sm:text-sm font-medium text-gray-500 hover:text-gray-800 transition-colors">
                <svg class="size-3 sm:size-4" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M15 19l-7-7 7-7"/></svg>
                Back to Home
            </a>
        </div>
        ...
        <div class="flex-1 flex flex-col justify-center max-w-md mx-auto w-full">
            ...
            <form method="POST" action="{{ route('caterer.login') }}" class="space-y-4 sm:space-y-6">
                @csrf
                <div>
                    <label class="block text-sm sm:text-base font-semibold text-gray-900 mb-1.5 sm:mb-2">Email Address</label>
                    <input type="email" name="email" value="{{ old('email') }}" required placeholder="you@example.com"
                        class="w-full px-3 sm:px-4 py-2.5 sm:py-4 rounded-lg sm:rounded-xl bg-gray-50 border {{ $errors->has('email') ? 'border-red-400' : 'border-gray-200' }} text-gray-900 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#111827] transition-all">
                </div>
                ...
                <div>
                    ...
                    <div class="relative">
                        <input id="password" type="password" name="password" required placeholder="Enter your password"
                            class="w-full px-3 sm:px-4 py-2.5 sm:py-4 rounded-lg sm:rounded-xl bg-gray-50 border {{ $errors->has('password') ? 'border-red-400' : 'border-gray-200' }} text-gray-900 text-sm sm:text-base focus:outline-none focus:ring-2 focus:ring-[#111827] transition-all pr-10 sm:pr-12">
                ...
                <button type="submit" class="w-full py-2.5 sm:py-4 rounded-lg sm:rounded-xl bg-[#111827] text-white text-sm sm:text-lg font-bold hover:bg-gray-800 transition-all shadow-md mt-2 sm:mt-4">
                    Sign In
                </button>
            </form>
            ...
            <div class="space-y-2 sm:space-y-4 text-center">
                <p class="text-gray-600 font-medium text-xs sm:text-base">Don't have an account? <a href="{{ route('caterer.register') }}" class="text-[#f44e08] font-bold hover:underline ml-1">Join as Caterer</a></p>
                <p class="text-gray-600 font-medium text-xs sm:text-base">Are you a client? <a href="{{ route('login') }}" class="text-[#f44e08] font-bold hover:underline ml-1">Sign in here</a></p>
            </div>
        </div>
    </div>

    {{-- Right side --}}
    <div class="hidden lg:flex lg:w-1/2 bg-[#111827] flex-col justify-center px-8 lg:px-20 py-10 text-white">
        <div class="max-w-lg">
            <h2 class="text-3xl lg:text-4xl font-bold leading-tight mb-4 lg:mb-6">Grow Your Catering Business</h2>
            <p class="text-base lg:text-xl text-gray-400 leading-relaxed mb-8 lg:mb-12">Connect with clients in Tagum City. Manage your bookings, showcase your specialties, and expand your reach.</p>
            <ul class="space-y-4 lg:space-y-6">
                ...
                <li class="flex items-start gap-3 lg:gap-4">
                    <div class="bg-white/10 rounded-full p-1 mt-0.5 lg:mt-1 shrink-0"><svg class="size-3 lg:size-5 text-gray-300" fill="none" stroke="currentColor" stroke-width="3" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" d="M5 13l4 4L19 7"/></svg></div>
                    <span class="text-base lg:text-xl font-medium text-gray-200">{{ $benefit }}</span>
                </li>
                @endforeach
            </ul>
        </div>
